// 8.0.0.3319. Generated 10/27/2016 5:01:08 PM UTC
...
$axure.internal(function($ax) {
    ...
    var _isSuppressedEvent = function(id, event) {
        var events = _suppressedEvents[id];
        return events && events.indexOf(event) != -1;
    }
...
//                if (easingIn === 'flip') _bringPanelStateToFront(id, stateId);
    ...
    var _setCurrFocus = function(id) {
        if(id) {
            ...
            window.setTimeout(function () {
                $ax.event.addSuppressedEvent($ax.repeater.removeSuffixFromElementId(id), 'OnFocus');
                $jobj(id).focus();
            }, 0);
        }
    }
